...
export interface FocusSnapshot { goal: string; notes: string[]; demo: boolean; matchups?: MatchupPlan[] }
  ...
  return <main className="focus-window">
    ...
    {data?.matchups?.length && <section className="focus-matchup" aria-label="Ficha de matchup">
      ...
      {(() => { const matchup = data.matchups.find(item => item.opponent === selectedOpponent) ?? data.matchups[0]; return <>
        ...
        <small className="focus-matchup-source">{matchup.sourceLabel} · escolha feita por você, sem leitura da partida atual.</small>
      </> })()}
    </section>}
