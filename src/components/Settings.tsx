...
import { Activity, Download, FolderSearch, Link2, Upload } from 'lucide-react'
import type { AppSettings } from '../domain/types'
import type { Library } from '../domain/library'
import { downloadJson, desktop } from '../services/library'
import { Heading } from './ui'

export function Settings({ settings, data, busy, onSave, onSync, onTest, onImport, onSwitchAccount }: { settings: AppSettings; data: Library; busy: boolean; onSave: (value: AppSettings) => void; onSync: (name: string, tag: string, key: string, count: number) => Promise<void>; onTest: (name: string, tag: string, key: string) => Promise<void>; onImport: (file: File) => Promise<void>; onSwitchAccount: () => Promise<void> }) {
  ...
  return <div className="page settings-page"><Heading eyebrow="SEU COACH, DO SEU JEITO" title="Configurações" />
    ...
    <section className="panel"><div className="section-icon"><Download size={22} /><div><h2>Cópia do seu histórico</h2><p>Exporte ou restaure partidas, revisões e anotações neste computador.</p></div></div><div className="form-actions"><button className="secondary" disabled={busy || !data.matches.length} onClick={() => downloadJson('jax-coach-historico.json', data)}><Download size={16} />Exportar histórico</button><button className="secondary" disabled={busy} onClick={() => fileInput.current?.click()}><Upload size={16} />Importar backup</button><input ref={fileInput} type="file" accept="application/json,.json" hidden onChange={async e => { const file = e.target.files?.[0]; e.currentTarget.value = ''; if (file) await onImport(file).catch(() => {}) }} /></div><p className="field-help">O arquivo não contém a chave Riot. Importar substitui o histórico pessoal depois de uma confirmação.</p><div className="account-switch"><strong>Trocar de conta</strong><span>Limpa somente o histórico pessoal deste computador. Exporte antes se quiser manter uma cópia.</span><button className="secondary" disabled={busy || !data.matches.length} onClick={() => void onSwitchAccount().catch(() => {})}>Limpar e conectar outra conta</button></div></section>
  </div>
}
