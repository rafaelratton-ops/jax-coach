...
export function Overview({ data, jaxOnly, demo, onMatch, onSettings, onFocus }: { data: Library; jaxOnly?: boolean; demo: boolean; onMatch: (m: MatchSummary) => void; onSettings: () => void; onFocus: () => void }) {
  ...
  return <div className="page">
    ...
    <aside className="panel focus-card"><div className="icon-box"><Monitor size={22} /></div><p className="eyebrow">PARA O SEGUNDO MONITOR</p><h2>Um foco de cada vez.</h2><p>{data.goal}</p><div className="hint"><BookOpen size={16} /><span>Seu objetivo e anotações anteriores ficam fixos. Nenhum evento da partida atual é consultado.</span></div><button className="secondary" onClick={onFocus}>Abrir painel de foco <ArrowRight size={16} /></button></aside></div>
